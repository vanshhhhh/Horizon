...
def getData2():
    ...
    rotationalAvg = rotationalAvg.set_index('Carrington Rotation')
...
def getData3():
    ...
    dailyFluxValues = dailyFluxValues.set_index('Carringtonrotation')
    dailyFluxValues = dailyFluxValues.drop(['Date', 'Time', 'Julian day'], axis = 1)
...
st.sidebar.image("./assets/images/team-logo.jpg", width = 150)
st.sidebar.title("Horizon")
st.sidebar.write("This is a data visualization tool for the analysis of Solar Flux data.")
rad = st.sidebar.radio('Navigation Menu',['Solar Flux Visualization', 'About Us'])
if rad =='About Us':
    st.title("About Us")
...
if rad == 'Solar Flux Visualization':
    ...
    if options == "Monthly Averages":
        ...
        try:
            ...
            st.warning(e)
    if options == "Rotational Averages":
